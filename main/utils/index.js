...
export const check = (data, checkTemplate) => {
  let err = [];
  if (!data)
    return;
  for (let key in checkTemplate) {
    if (!data[key] && checkTemplate[key].must) {
      err.push({msg: `缺少参数 ${key} `});
    }
  }
  for (let key in data) {
    if (!checkTemplate[key]) {
      err.push({msg: `不允许参数  ${key} `});
    } else {
      if (typeof data[key] != (checkTemplate[key].type || "string")) {// NOTE: 默认参数类型为字符串
        err.push({msg: `参数 ${key} 类型有误，类型必须为 ${checkTemplate[key].type} `});
        break;
      }
      if (checkTemplate[key].regexp) {
        if (!new RegExp(checkTemplate[key].regexp).test(data[key])) {
          err.push({msg: `参数 ${key} 格式有误，格式必须为 ${checkTemplate[key].regexp} `});
        }
      }
      if (checkTemplate[key].min || checkTemplate[key].max) {
        if (data[key].length > checkTemplate[key].max) {
          err.push({msg: `参数 ${key} 长度不能超过 ${checkTemplate[key].max} `});
        }
        if (data[key].length < checkTemplate[key].min) {
          err.push({msg: `参数 ${key} 长度不能小于 ${checkTemplate[key].max} `});
        }
      }
    }
  }
